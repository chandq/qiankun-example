...
const getRoutePath = path => {
  console.log('basename-path: ', basename,path, `${basename}${path}`.replace('//', '/') )
  return process.env.NODE_ENV === 'development' ? `${basename}${path}`.replace('//', '/') : `${process.env.PUBLIC_URL}${path}`.replace('//', '/')
}
...
const parseRoutes = (routesConfig, app) => {
  const config = routesConfig.map(
    ({ path, name, layout: Layout, redirect, children, component: Component, ...dynamics }) => {
      ...
      redirect && console.log('compo redirect: ', redirect, getRoutePath(redirect))
      return redirect ? (
